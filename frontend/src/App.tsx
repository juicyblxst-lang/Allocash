...
export default function App() {
  ...
  async function createSmartAccount() {
    ...
    try {
      ...
      setStatus('Smart account created. Every later financial action requires an explicit owner wallet transaction.');
  ...
  async function createPreset() {
    ...
    try {
      setStatus('Review complete. Confirm the immutable preset in your wallet…');
      ...
      if (client) { await client.waitForTransactionReceipt({hash}); const next = await client.readContract({address:vault,abi:vaultAbi,functionName:'nextPresetId'}); setPresetId(next-1n); }
